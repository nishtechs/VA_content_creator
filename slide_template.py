...
def render_slide(
    ...
    if visual_html:
        ...
        # Strip any duplicate typewriter code blocks (code-wrap containing codeTarget) generated by the LLM
        visual_html = re.compile(
            r"<div class=['\"]code-wrap[^'\"]*['\"][^>]*>.*?<code[^>]*id=['\"]codeTarget['\"][^>]*>.*?</code>.*?</div>",
            re.DOTALL
        ).sub("", visual_html).strip()
    if custom_css:
